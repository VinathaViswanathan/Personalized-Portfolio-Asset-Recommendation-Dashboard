...
def demographic_score(customer_id, customer_df, asset_df):
    ...
    for cat in asset_df["assetCategory"].unique():
        ...
        sim = 1 - np.sqrt(np.sum(weights * (user_vector - avg_vector) ** 2)) / np.sqrt(
            np.sum(weights * np.array([3, 3, 1, 1]) ** 2))
...
def hybrid_recommendation(customer_id, rating_matrix, pred_df, rating_df, asset_df,
    ...
    bought_assets = rating_df[rating_df['customerID'] == customer_id]['ISIN'].unique() if not rating_df[
        rating_df['customerID'] == customer_id].empty else []
...
def precision_recall_at_n(pred_func, train_df, test_df, rating_matrix, rating_df, asset_df, customer_df,
                          limit_prices_df, weights, pred_ratings, N):
    ...
    for _, row in test_df.iterrows():
        try:
            ...
            recs = pred_func(u, rating_matrix, pred_ratings, rating_df, asset_df, customer_df, limit_prices_df, weights,
                             top_n=N)
...
def main():
    ...
    st.write(
        "An improved hybrid recommendation system leveraging the FAR-Trans dataset, combining collaborative filtering, enriched content-based filtering, and demographic matching.")
